...
+ (void)drawString:(NSString *)text atPoint:(CGPoint)location withAnchor:(int)anchor
{
	if ([text length])
	{
		...
		textRect.origin.x = location.x - textRect.size.width / 2; //Set textRect's origin based on location argument, alter according to width (x-value) size of textRect
		textRect.origin.y = location.y - textRect.size.height / 2;
		
        //This is a good way to implemenet switch statements with Strings
		
        switch (anchor) {
                    //Why adding to y value?
...
+ (void)drawHashMarksInRect:(CGRect)bounds originAtPoint:(CGPoint)axisOrigin scale:(CGFloat)pointsPerUnit
{
    ...
    //Can store BOOL values as integers. 
	int started = NO;
	int stillGoing = YES;
...
+ (void)drawAxesInRect:(CGRect)bounds originAtPoint:(CGPoint)axisOrigin scale:(CGFloat)pointsPerUnit
{
	...
	CGContextAddLineToPoint(context, bounds.origin.x+bounds.size.width, axisOrigin.y); //Add xline
	CGContextMoveToPoint(context, axisOrigin.x, bounds.origin.y);
	CGContextAddLineToPoint(context, axisOrigin.x, bounds.origin.y+bounds.size.height); //Add yLine
	CGContextStrokePath(context);
